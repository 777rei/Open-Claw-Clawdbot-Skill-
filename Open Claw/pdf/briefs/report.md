...
|-----------|-------|------|
| `NotoSerifSC` | **Chinese body text (primary)** |  `/usr/share/fonts/Noto_Serif_SC/NotoSerifSC-Regular.ttf` |
| `NotoSerifSC-Bold` | **Chinese headings (primary)** |  `/usr/share/fonts/Noto_Serif_SC/NotoSerifSC-Bold.ttf` |
| `Noto Sans SC` | Chinese fallback / legacy | `/usr/share/fonts/truetype/chinese/NotoSansSC[wght].ttf` |
| `Noto Sans SC` | Chinese alternative (sans-serif) | `/usr/share/fonts/truetype/chinese/NotoSansSC[wght].ttf` |
...
> **Font file locations note:** On macOS, font paths may be under `~/.openclaw/workspace/fonts/truetype/` or `~/Library/Fonts/` instead of `/usr/share/fonts/`. The registration code below uses a `FONT_DIR` variable — set it to the correct base path for your system.
...
if _IS_MAC:
    FONT_DIR = os.path.expanduser('~/.openclaw/workspace/fonts/truetype')
    NOTO_SERIF_DIR = f'{FONT_DIR}/chinese'
else:  # Linux
    FONT_DIR = '/usr/share/fonts/truetype'
    NOTO_SERIF_DIR = '/usr/share/fonts/Noto_Serif_SC'

# Chinese fonts — Noto Serif SC (primary, serif, high quality)
pdfmetrics.registerFont(TTFont('NotoSerifSC', f'{NOTO_SERIF_DIR}/NotoSerifSC-Regular.ttf'))
pdfmetrics.registerFont(TTFont('NotoSerifSC-Bold', f'{NOTO_SERIF_DIR}/NotoSerifSC-Bold.ttf'))
# Chinese fonts — fallback / legacy
pdfmetrics.registerFont(TTFont('Noto Sans SC', f'{FONT_DIR}/chinese/NotoSansSC[wght].ttf'))
pdfmetrics.registerFont(TTFont('Noto Sans SC', f'{FONT_DIR}/chinese/NotoSansSC[wght].ttf'))
pdfmetrics.registerFont(TTFont('SarasaMonoSC', f'{FONT_DIR}/chinese/SarasaMonoSC-Regular.ttf'))
...
# Symbol/Formula font
pdfmetrics.registerFont(TTFont('DejaVuSans', f'{FONT_DIR}/dejavu/DejaVuSansMono.ttf'))
...
cn_style = ParagraphStyle(name="CN", fontName="NotoSerifSC", fontSize=10.5, leading=18)
...
# pdfmetrics.registerFont(TTFont('Noto Sans SC', '/usr/share/fonts/truetype/chinese/NotoSansSC[wght].ttf'))
# registerFontFamily('Noto Sans SC', normal='Noto Sans SC', bold='Noto Sans SC')
...
| chart | matplotlib figure saved as PNG → `Image()` flowable | Generate chart in-script, `fig.savefig()` → embed. Set `plt.rcParams['font.sans-serif']=['Noto Sans SC']` for CJK. Always `tight_layout()`. **Must follow `typesetting/charts.md` rules**: delete top/right spines, dashed grid 20% opacity, donut default for pie, anti-stacking labels. |
...
- [ ] **⚠️ MANDATORY font fallback**: `install_font_fallback()` called after font registration — **failure to call this is the #1 cause of Helvetica garbled text (乱码) in CJK documents**
