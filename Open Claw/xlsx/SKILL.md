...
```bash
bash "$XLSX_SKILL_DIR/env_setup/env_check.sh"
```
...
After setup, the following variables must be available:

| Variable | Value | Description |
|----------|-------|-------------|
| `XLSX_SKILL_DIR` | `<skill_directory>` | Skill root directory (parent of this file) |
| `XLSX_FONTS_DIR` | *(set by setup script)* | User font directory (set after `setup_mac_linux.sh` or `setup_windows.ps1` runs) |

**macOS / Linux:**
```bash
XLSX_SKILL_DIR="<skill_directory>"
export XLSX_SKILL_DIR
bash "$XLSX_SKILL_DIR/env_setup/setup_mac_linux.sh"
# XLSX_FONTS_DIR is exported by the setup script (points to user font dir)
```

**Windows (PowerShell):**
```powershell
$env:XLSX_SKILL_DIR = "<skill_directory>"
powershell -ExecutionPolicy Bypass -File "$env:XLSX_SKILL_DIR\env_setup\setup_windows.ps1"
# XLSX_FONTS_DIR is set by the setup script
```
